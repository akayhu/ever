 ...
 * @param {object} param
 */
export const tagsToGetFileArr = ({ fileId, mediaType, convertType, target, page }) => {
  if (!mediaTypeMap.hasOwnProperty(mediaType) || !tagsMap[mediaType].hasOwnProperty(convertType)) {
  ...
  return tagsMap[mediaType][convertType].map(fileTag => ({ fileId, protocol: 'https', fileTag, target, page }));
};
