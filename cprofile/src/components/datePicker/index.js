...
class DatePickerWrapper extends Component {
	...
	handleChange = (value, name) => {
		this.setState(
			...
			() => {
				this.setState(state => ({
					endYear:
						state.endYear < state.startYear ? state.startYear : state.endYear,
					endMonth:
	...
	renderYear = name => {
		...
		if (name === 'endYear') {
			...
			for (let i = 1911; i <= today.getFullYear(); i += 1) {
				year.push(i);
			}
		}
	...
	renderSeniority = () => {
		...
		const endTime = stillWorking
			...
			: moment({
					...
					month: endMonth || defaultTime.endMonth,
			  });
	...
	toggleStillWorking = e => {
		this.setState(
			...
			() => {
				this.setState(state => ({
					endYear: state.stillWorking ? today.getFullYear() : state.endYear,
					endMonth: state.stillWorking ? today.getMonth() + 1 : state.endMonth,
				}));
			}
		);
	};
