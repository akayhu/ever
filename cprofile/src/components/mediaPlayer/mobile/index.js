...
const getMappingData = fileFromDocApiSrc => {
	...
	const docApiMap = {
		VIDEO: {
			tag: ['720p'],
			template: (
				<video src={fileFromDocApiSrc[0]} controls autoPlay="autoplay" />
			),
		},
		AUDIO: {
			tag: [],
...
class MobileMediaPlayer extends Component {
	...
	_handleClick(e) {
		...
		if (!this.state.transformed) {
			getFileUrl(
				this.props.property.fileid,
				this.state.tagType,
				getMappingData()[this.state.tagType].tag
			).done(function(res) {
				that.state.carrier = getMappingData(res[0].url)[
					that.state.tagType
				].template;
				that.setState({
					transformed: true,
	...
	render() {
		...
		if (
			typeof this.props.property.convertstatus !== 'undefined' &&
			this.props.property.convertstatus !== '1'
		) {
			return (
			...
			return (
				<div styleName="block">
					<a href={this.props.property.url} target="_blank">
					...
					<div>
						<iframe
							...
							height="267.5"
							src={
								'http' + '://www.youtube.com/embed/' + this.props.property.file
							}
						/>
					</div>
				</div>
			...
			let blockStyle = this.state.transformed
				? ''
				: this.state.tagType.toLowerCase();
			return (
