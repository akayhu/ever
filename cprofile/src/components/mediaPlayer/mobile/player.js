		...
		return (
			<div ref="player" styleName="player" tabIndex="1">
				<div ref="playground" styleName="playground">
					{this.state.loading && (
						<div styleName="loading" className="ui loading" />
					)}
					<img ref="play_image" />
				</div>
				<div styleName="controls">
					<span styleName="direction">
						<i onClick={e => this.previous(e)} className="icon left_arrow" />
					</span>
					<span styleName="count">
						<input
							value={this.state.currentIndex + 1}
							onChange={e => this.jump(e)}
						/>
						/{this.state.imagesCount}
					</span>
					<span styleName="direction">
						<i onClick={e => this.next(e)} className="icon right_arrow" />
					</span>
				</div>
			</div>
		);
