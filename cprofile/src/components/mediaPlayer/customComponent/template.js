...
const LoadingBlock = CSSModules(
	parent => {
		const removeBlock = () => {
			parent.props.blockProps.onRequestRemove(
				parent.props.block.getKey(),
				parent.state.props.id
			);
		};

		return (
			<div styleName="loading-preset">
				<div styleName="close" onClick={removeBlock} />
...
const ProcessingBlock = CSSModules(
	parent => {
		return (
			<div styleName="block">
				<div styleName="loading-preset">
					<div styleName="play-icon process" />
			...
			<div styleName="block">
				<div>
					...
					{typeof props.convertstatus !== 'undefined' ? (
						<div
							styleName="mediaBlcok"
							style={{ background: 'url(' + props.src + ') no-repeat center ' }}
						>
							<div styleName="play-icon video" />
...
const DocumentBlock = CSSModules(
	({ parent, props }) => {
		const removeBlock = () => {
			parent.props.blockProps.onRequestRemove(
				parent.props.block.getKey(),
			...
			<div styleName="block">
				<div>
					...
					<div
						...
						style={{ background: 'url(' + props.src + ') no-repeat center' }}
					>
						<div styleName="play-icon document" />
...
const HyperLinkBlock = CSSModules(
	({ parent, props, onError, hyperImgError }) => {
		const removeBlock = () => {
			parent.props.blockProps.onRequestRemove(
				parent.props.block.getKey(),
				parent.state.props.id
