...
class FileUploader extends Component {
	constructor(props) {
		...
		for (let key in mediaInfo) {
			...
			for (let i in mediaInfo[key].multiAction) {
				tagArrMap[key].push(mediaInfo[key].multiAction[i].tag);
			}
		}
		this.handleClick = e => {
		...
		this.generatorProcess = function*(ID, signatureData) {
			if (this.fileList[ID]) {
				let signature = yield getSignature(
					this.fileList[ID].originFile,
					signatureData
				);
				this.fileList[ID].signature = signature;
				this.fileList[ID].status = 'uploading';
				//this.fileList[ID].snapTag = snap;
				if (this.props.getSignatureDone)
					this.props.getSignatureDone(this.logObject(this.fileList[ID]));

				let uploadS3 = yield uploadToS3(
					this.fileList[ID].originFile,
					signature
				);
				this.fileList[ID].status = 'uploadDone';
				if (this.props.uploadToS3Done)
					this.props.uploadToS3Done(this.logObject(this.fileList[ID]));

				if (!this.props.dontWaitSuccess) {
					this.fileList[ID].status = 'transforming';
					this.fileList[ID].transformedFile = yield waitUrlSuccess(
						signature.fileId,
						this.fileList[ID].type,
						tagArrMap[this.fileList[ID].type]
					);
					this.fileList[ID].status = 'transformDone';
					if (this.props.urlTransformDone)
						this.props.urlTransformDone(this.logObject(this.fileList[ID]));
				}
	...
	_uploadProcess(AtomicType, f) {
		...
		signatureData.extra = mediaInfo[AtomicType];
