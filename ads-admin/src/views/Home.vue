<template>
  <div :class="getWrapperMainClass">
    ...
    >
      ...
      <div>
        <div class="title">歡迎來到104廣告平台後台</div>
        <p>
          如欲開通服務，請至
          <a
        ...
        <div class="button_block">
          <button @click="loginFunc" class="button_bg_blue_large">登入</button>
        </div>
      </div>
    </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    // 是否可登入
    const getAccessible = computed(() => getUserStatus.value.accessible);
    // Eportal Url
    const getEportalUrl = computed(
      () => `http:${process.env.VUE_APP_PORTAL_URL}`
    );
    // Wrapper Main Class
    const getWrapperMainClass = computed(() => {
      if (getUserStatus.value.type !== 0) {
        switch (getRouterName.value) {
          case "Home":
          case "LoginHome":
