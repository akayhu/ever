...
class MBeagiverResumeclinic extends Component {
	constructor(props) {
		super(props);
		this.state = {
			iframeHeight: '1000',
			...
			switch (this.props.location.query.type) {
				...
				case 'info': // 活動說明
					...
					if (canUseDOM) { // 活動說明文章，固定aid
						window.open('/m/activity/6b857787-b8b0-4ab3-8085-4d9c730d1050');
					}
					break;
	...
	onMessage() {
		const resumeclinicRouter = '/m/104beagiver/resumeclinic';
			...
			if (arguments[0].data.height) { // iframe高
				...
				switch (arguments[0].data.menu) {
					...
					case 'stream':
						...
						break;
					case 'info':
						window.open('/m/activity/6b857787-b8b0-4ab3-8085-4d9c730d1050'); // 活動說明文章，固定aid
						window.history.replaceState({}, '', `${resumeclinicRouter}?type=${arguments[0].data.menu}`);
						break;
					default:
						window.history.replaceState({}, '', resumeclinicRouter);
...
export default CSSModules(MBeagiverResumeclinic, css, {allowMultiple: true});
