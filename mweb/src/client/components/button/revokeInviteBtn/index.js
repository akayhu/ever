...
// actions
import {triggerRevoke} from 'src/client/actions/profile';
// components
import { DropdownMenu, DropdownTarget, DropdownList } from 'c_wap_module';

class RevokeInviteBtn extends Component {
	constructor(props) {
		super(props);
		this.state = {
			loading: false,
	...
	handleClick() {
		const {targetPid, triggerRevoke} = this.props;

		this.setState({loading: true});
		triggerRevoke(targetPid).then(this.updateButton);
	}
	updateButton(res) {
		if (res !== true) {
			const {msg, canRetry} = res;
			this.setState({
				loading: false,
				disabled: !canRetry,
			});
			...
			<DropdownMenu className={ this.getButtonStyle() }>
				...
				<DropdownList>
					<ul styleName="other_actions">
						<li onClick={ this.handleClick }>收回交友邀請</li>
					</ul>
				</DropdownList>
			</DropdownMenu>
		);
