...
export const CHANGE_CHANNEL_TAB = 'CHANGE_CHANNEL_TAB';           // for 個別頻道內部橘色頁籤（包含 subNav、左側）
export const CHANGE_CHANNEL_LIST_TAB = 'CHANGE_CHANNEL_LIST_TAB'; // for 頻道列表橘色頁籤
...
export const changeChannelTab = tab => dispatch =>
	dispatch({type: CHANGE_CHANNEL_TAB, tab});
...
export function changeListTab(tab) {
	return (dispatch, getState) => {
		...
		dispatch({type: CHANGE_CHANNEL_LIST_TAB, tab});
...
export function actionMap(tab) {
	switch (tab) {
		...
		case 'channelAdmin':
			return ChannelApi.getMediaAdminList;
