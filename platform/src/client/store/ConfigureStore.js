...
export default function ConfigureStore(rootReducer, middlewares, initialState) {
	...
	let store = null;
	const defaultMiddlewares = [thunk.withExtraArgument(), checkType(), _logger(loggerConfig)];
	const mergedMiddlewares = defaultMiddlewares.concat(middlewares);
	const applyMiddlewareWrap = applyMiddleware.apply(undefined, mergedMiddlewares);
