	...
	render() {
		...
		return (
	        ...
	        <link rel="shortcut icon" href={clientConfig.params.staticPlatformUrl+"/favicon.ico"} />
				...
				<body>
					{['dev','lab'].indexOf(clientConfig.env) !== -1 && this.props.error_message && <div style={{width: '80%', margin: '100px auto 0', fontWeight: 'bold'}}>This is Dev Env Only</div>}
					{['dev','lab'].indexOf(clientConfig.env) !== -1 && this.props.error_code && <div style={{width: '80%', margin: '0 auto', fontWeight: 'bold'}}>Error code : {this.props.error_code}</div>}
					{['dev','lab'].indexOf(clientConfig.env) !== -1 && this.props.error_message && <div style={{width: '80%', margin: '0 auto', fontWeight: 'bold'}}>Error message : {this.props.error_message}</div>}
					{['dev','lab'].indexOf(clientConfig.env) !== -1 && this.props.error_stack && <pre style={{width: '80%', margin: '100px auto 0', fontWeight: 'bold'}}>{this.props.error_stack}</pre>}
